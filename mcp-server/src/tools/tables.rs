...
pub fn add_table(store: &mut WorkbookStore, input: AddTableInput) -> Result<String, anyhow::Error> {
    ...
    let cols: Vec<zavora_xlsx::TableColumn> = input.columns.iter().map(|n| zavora_xlsx::TableColumn::new(n)).collect();
    let mut table = zavora_xlsx::Table::new();
    table.set_columns(&cols);
    if let Some(ref style) = input.style {
        let s = style.to_lowercase();
        if s.contains("medium") {
            let n: u8 = s.chars().filter(|c| c.is_ascii_digit()).collect::<String>().parse().unwrap_or(2);
            table.set_style(zavora_xlsx::TableStyle::Medium(n));
        } else if s.contains("light") {
            let n: u8 = s.chars().filter(|c| c.is_ascii_digit()).collect::<String>().parse().unwrap_or(1);
    ...
    entry.data.worksheet(idx).map_err(|e| anyhow::anyhow!("{e}"))?.add_table(r1, c1, r2, c2, &table)?;
    Ok(success_no_data(&format!("Table added to '{}'", input.sheet_name)))
}
